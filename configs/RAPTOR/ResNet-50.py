_base_ = [
    '../_base_/datasets/Military_RSOD.py',  # <-- 使用相对路径
...
# NOTE: This baseline config does not need custom_imports or custom_hooks
# because it does not use any custom modules.

model = dict(
    type='OrientedRCNN',
    
    # ==================== This is the ONLY changed section ====================
    # 1. --- Backbone is now standard ResNet-50 ---
    backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4,
    ...
    # 2. --- Neck is now a standard FPN compatible with ResNet-50 ---
    neck=dict(
        type='FPN',
    ...
    # --- RPN and RoI heads are IDENTICAL to your custom model's config ---
    rpn_head=dict(
    ...
    roi_head=dict(
        ...
        bbox_head=dict(
            ...
            roi_feat_size=7,
            num_classes=37, # For DIOR-R dataset
            bbox_coder=dict(
    ...
    # --- Train and test configurations are IDENTICAL ---
    train_cfg=dict(
        rpn=dict(
            assigner=dict(
        ...
        rcnn=dict(
            assigner=dict(
                ...
                iou_calculator=dict(type='RBboxOverlaps2D'), ignore_iof_thr=-1),
            sampler=dict(
...
# --- Data configurations are IDENTICAL ---
img_norm_cfg = dict(
