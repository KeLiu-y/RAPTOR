...
class LightweightBlock(nn.Module):
    """A lightweight bottleneck block using ARC for feature extraction."""
    def __init__(self, in_channels, mid_channels, out_channels):
        super().__init__()
        self.conv1 = ConvModule(
            in_channels, mid_channels, kernel_size=1, act_cfg=dict(type='ReLU')
        )
        self.arc_conv = ARConv(
            mid_channels, mid_channels, kernel_size=3
        )
        self.conv2 = ConvModule(
            mid_channels, out_channels, kernel_size=1, act_cfg=None
        )
...
class ARCRotatedBBoxHead(RotatedConvFCBBoxHead):
    """
    Final robust implementation of the lightweight HAPHead template.
    """
    def __init__(self, num_refine_convs=1, mid_channels=64, **kwargs):
        # 1. Call parent __init__ FIRST.
        #    This is the most robust way. It uses the config to build a
        #    standard head, passing all internal assertion checks.
        super().__init__(**kwargs)

        # 2. Now, OVERWRITE the layers with our custom lightweight versions.

        # Create our custom "coarse" layer.
        self.coarse_conv = LightweightBlock(
            self.in_channels,
            mid_channels,
            self.conv_out_channels
        )

        # Re-build and overwrite the "refinement" branches using lightweight DWConv.
        # This replaces the heavy standard convs that the parent built.
        self.cls_convs = nn.ModuleList()
        self.reg_convs = nn.ModuleList()
        for i in range(num_refine_convs):
            self.cls_convs.append(
                DWConv(self.conv_out_channels, self.conv_out_channels, k=3)
            )
    ...
    def forward(self, x):
        """Forward pass following the HAPHead template structure."""
        # 1. Coarse alignment with the lightweight block.
        coarse_feat = self.coarse_conv(x)

        # 2. Refine features in separate branches using lightweight DWConvs.
        cls_feat = coarse_feat
        reg_feat = coarse_feat
        ...
        # 3. Apply pooling (CRITICAL for FLOPs).
        ...
        # 5. Apply shared FC layers if configured.
        if self.num_shared_fcs > 0:
            for fc in self.shared_fcs:
                cls_feat = self.relu(fc(cls_feat))
                reg_feat = self.relu(fc(reg_feat))

        # 6. Final prediction layers (built by parent).
