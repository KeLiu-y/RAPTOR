...
class RotatedBBoxHead(BaseModule):
    ...
    # In your rotated_bbox_head.py file
    ...
    def loss(self,
        ...
        if bbox_pred is not None:
            # ⬇️ --- ADD THE DEBUGGING BLOCK HERE --- ⬇️
            # Check if the predictions contain any NaN or Inf values
            has_nan = torch.isnan(bbox_pred).any()
            has_inf = torch.isinf(bbox_pred).any()

            if has_nan or has_inf:
                print("\n" + "="*50)
                print("‼️‼️‼️  DEBUGGING: Invalid values detected in bbox_pred!  ‼️‼️‼️")
                print(f"Contains NaN: {has_nan}")
                print(f"Contains Inf: {has_inf}")
                # Printing min/max can help understand the scale of the explosion
                print(f"bbox_pred min value: {torch.min(bbox_pred)}")
                print(f"bbox_pred max value: {torch.max(bbox_pred)}")
                print(f"bbox_pred mean value: {torch.mean(bbox_pred)}")
                print("="*50 + "\n")
            # ⬆️ --- END OF DEBUGGING BLOCK --- ⬆️
