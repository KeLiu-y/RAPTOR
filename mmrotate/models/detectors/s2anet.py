...
class S2ANet(RotatedBaseDetector):
    ...
    def __init__(self,
                 ...
                 train_cfg=None,
                 test_cfg=None,
                 pretrained=None,  # 这个参数会被忽略，但为了安全先保留
                 init_cfg=None):   # <-- 1. 添加 init_cfg=None
        
        # 2. 将 init_cfg 传递给父类
        super(S2ANet, self).__init__(init_cfg) 

        # 3. 移除或注释掉旧的 backbone.pretrained = pretrained
        # if pretrained:
        #     warnings.warn('pretrained is deprecated, please use "init_cfg" instead')
        #     backbone.pretrained = pretrained
            
        self.backbone = build_backbone(backbone) # <-- 现在 build_backbone 会正确处理 init_cfg
